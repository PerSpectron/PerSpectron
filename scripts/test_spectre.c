...
const unsigned int num_neurons_hidden = 2;
	...
	data = fann_read_train_from_file(train_data_file);
	...
	*/
	...
	for(i = 0; i < fann_length_train_data(data); i++)
	{
		...
		i, calc_out[0], output,
		 data->output[i][0],
		 fann_abs(output - data->output[i][0]));
		//	   data->input[i][0], data->input[i][1], calc_out[0], data->output[i][0],
			//   fann_abs(calc_out[0] - data->output[i][0]));
		if(output - data->output[i][0]==0)
			correct++; 
	}
	...
	//fann_save_train_to_fixed(data, "trained.data", decimal_point);
